...
function cacheElements() {
    ...
    deleteCurrentSessionButton =
        document.getElementById("deleteCurrentSession");
...
function wireEvents() {
    ...
    deleteCurrentSessionButton.addEventListener(
        "click",
...
function migrateLegacyData() {
    ...
    const legacyCount =
        localStorage.getItem("counterApp.count") ??
    ...
    */
    try {
        ...
        if (Array.isArray(legacyHistory) && legacyHistory.length > 0) {
            ...
            const values = appData.current.values.map(
                (entry) => entry.value
            );
...
function ensureValidData() {
    ...
    if (!isValidTimestamp(appData.current.startedAt)) {
        appData.current.startedAt =
            appData.current.values[0].timestamp;
    }
...
function renderRecords() {
    ...
    highTime.textContent = formatTimestamp(
        appData.records.high.timestamp
    );
    lowValue.textContent = appData.records.low.value;
    lowTime.textContent = formatTimestamp(
        appData.records.low.timestamp
    );
}
...
function drawArchivedSparklines() {
    ...
    sessionList
        .querySelectorAll("canvas[data-session-id]")
        .forEach((canvas) => {
            const session = sessionsById.get(
...
function drawSparkline(canvas, values) {
    ...
    */
    if (minimum <= 0 && maximum >= 0) {
        const zeroY =
            padding +
            ((maximum - 0) / valueRange) * usableHeight;
        context.beginPath();
    ...
    values.forEach((value, index) => {
        ...
        const y =
            padding +
            ((maximum - value) / valueRange) * usableHeight;
        if (index === 0) {
            context.moveTo(x, y);
        }
        else {
    ...
    const lastY =
        padding +
        ((maximum - lastValue) / valueRange) * usableHeight;
    context.beginPath();
